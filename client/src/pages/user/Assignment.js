...
const Assignment = () => {
  ...
  const fetchData1 = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/v1/auth/getallassignments`);
  ...
  function countPendingObjects(AssignmentDetails) {
    for (let i = 0; i < totalAssigned; i++) {
      ...
      if (AssignmentDetails[i].status === "Submitted") submitcount++;
    }
  }
                ...
                <div className="row" key={index}>
                  <div className="assignment-small-container-1">
                    ...
                    <p className="assignment-date">
                      uploaded on {formatDate(assignment["updatedAt"])}
                    </p>
                  </div>
